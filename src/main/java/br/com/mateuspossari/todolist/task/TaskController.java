...
import br.com.mateuspossari.todolist.utils.Utils;
...
public class TaskController {
    ...
    @Autowired //armazenar o TaskModel
    ...
    // Update Task
    // http://localhost:8080/tasks/5344321863-cdsfds-554344856
    @PutMapping("/{id}")
    public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
        var task = this.taskRepository.findById(id).orElse(null);

        if (task == null) { // se task não existir
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Tarefa não encontrada!");
        }

        var idUser = request.getAttribute("idUser");

        if (!task.getIdUser().equals(idUser)) { // se existir o user, mas o ID for diferente, não será permitido
